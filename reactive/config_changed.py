...
from .main import create_sysctl, ephemeral_unmount, \
    finish_initialization, get_glusterfs_version, initialize_storage
import os
import subprocess
        ...
        if output_result.is_ok():
            ...
            if process_output_result.is_ok():
                ...
                log("Device {} formatting failed with error: {}. "
                    "Skipping".format(handle.device.dev_path,
                                      process_output_result.value), Error)
        else:
            # Failed
...
def check_for_sysctl() -> Result:
    ...
    if config.changed("sysctl"):
        ...
        if sysctl_dict is not None:
            create_sysctl(sysctl, sysctl_file)
            # Reload sysctl's
